...
// This library goes kinda overboard on types, to try and make sure we know exactly what kind of
// color we're talking about at each step in the process.
...
// ApplyCameraWhite performs white balancing. After this operation,
...
func ApplyCameraWhite(cn CameraNative, cameraWhite emath.Vec3) hdrcolor.RGB {
	return hdrcolor.RGB{
		R: cn.RGB.R / cameraWhite[0],
		G: cn.RGB.G / cameraWhite[1],
		B: cn.RGB.B / cameraWhite[2],
	}
}

// ApplyCameraToPCS does all the color correction, mapping a camera native
// RGB into Process Connection Space (PCS), CIE XYZ(D50).
func ApplyCameraToPCS(rgb hdrcolor.RGB, cameraToPCS emath.Mat3) hdrcolor.XYZ {
	xyz := cameraToPCS.Apply(emath.Vec3{rgb.R, rgb.G, rgb.B})
	return hdrcolor.XYZ{xyz[0], xyz[1], xyz[2]}
}

// MakeCameraToPCS creates the CameraToPCS matrix, a single transform
// from camera native space to CIE XYZ (D50). `asShotNeutral` and
// `forwardMatrix` are Adobe DNG terms.
func MakeCameraToPCS(asShotNeutral emath.Vec3, forwardMatrix emath.Mat3) emath.Mat3 {
	D := asShotNeutral.InvertDiag()
	return forwardMatrix.Mult(D)
}

// ToPCS uses the matrix to transform the color into PCS (Profile
// Connection Space), CIEXYZ(D50). You want to use the CameraToPCS
// matrix generated by the DNG SDK, it includes any needed white
// balance transforms. You prob want to then convert this XYZ color
// back down to an sRGB(D65) color with `ecolor.XYZToSRGB()`
func (cn CameraNative)ToPCS(cameraToPCS emath.Mat3) hdrcolor.XYZ {
	rgb := hdrcolor.RGB{cn.RGB.R, cn.RGB.G, cn.RGB.B}
	return ApplyCameraToPCS(rgb, cameraToPCS)
}
