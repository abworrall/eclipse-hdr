...
// In your OS, install buildchain and FFTW3 dev library:
//  $ sudo apt-get install build-essential
//  $ sudo apt-get install libfftw3-dev

// If you run into precision issues, because your underlying C
// platform doesn't think a 'double' can fit into a float64, read
// https://www.fftw.org/fftw3_doc/Precision.html and make changes to
// the library namein LDFLAGS, and all the `fftw_` prefixes in this
// file.
...
func NewFftwPlan(in, out FloatGrid) *FftwPlan {
	var (
...
// Need address of first elem in the array, to pass as a (double *) via C bindings
func (fg *FloatGrid) ptr2array() *float64 { return &fg.values[0] }
...
func transform_ev2normal(A FloatGrid) FloatGrid {
	width  := A.Dx()
	height := A.Dy()
	T      := A.NewFromThis()
...
func transform_normal2ev(A FloatGrid) FloatGrid {
	width  := A.Dx()
	height := A.Dy()
	T      := A.NewFromThis()
...
func make_compatible_boundary(F FloatGrid) {
	width  := F.Dx()
	height := F.Dy()
...
func solve_pde_fft(F FloatGrid, adjustBound bool) FloatGrid {
  ...
  for i:=0; i<width*height; i++ {
		if U.values[i] > max { max = U.values[i] }
  }
  ...
  log.Printf("solve_pde_fft: done\n")
	U.ToImg("006-solved-PDE", "006-solved-PDE.png")
