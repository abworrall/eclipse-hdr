...
import(
	...
	"github.com/fogleman/gg" // for image text; prefer https://pkg.go.dev/golang.org/x/image/font#Drawer
)
...
// ToImg drops a simple grayscale, based on the range of values in the grid, and gamma scaling the
...
func (fg *FloatGrid)ToImg(title, filename string) {
	...
	//log.Printf("Written grid file %s (scale{%f,%f}, size=%d,%d)\n", filename, min, max, fg.Dx(), fg.Dy())
}
